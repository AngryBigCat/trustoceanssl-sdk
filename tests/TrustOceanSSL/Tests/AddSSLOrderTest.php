<?php


namespace TrustOceanSSL\Tests;


use TrustOceanSSL\Request\AddSSLOrderRequest;

class AddSSLOrderTest extends TestCase
{
    public function testAddSSLOrderTest()
    {
        $params = [
            'pid' => 50,
            'csr_code'=>'-----BEGIN CERTIFICATE REQUEST-----

-----END CERTIFICATE REQUEST-----
',
          'period'=>'Annually',
          'unique_id'=>'un32ms0902',
          'dcv_method'=>'https',
          'contact_email'=>'1342479179@qq.com',
          'callback'=>'https://xays.top',
          'domains'=> 'www.xays.top',
          'renew'=>'yes',
//          'organization_name'=>'',
//          'organizationalUnitName'=>'<string>',
//          'registered_address_line1'=>'<string>',
//          'registered_no'=>'<string>',
//          'country'=>'<string>',
//          'state'=>'<string>',
//          'city'=>'<string>',
//          'postal_code'=>'<string>',
//          'organization_phone'=>'<string>',
//          'date_of_incorporation'=>'<string>',
//          'contact_name'=>'<string>',
//          'contact_title'=>'<string>',
//          'contact_phone'=>'schema type not provided'
            ];

//        $params = new AddSSLOrderRequest();
//        $params->pid = 50;
//        $params->csr_code = '-----BEGIN CERTIFICATE REQUEST-----

//-----END CERTIFICATE REQUEST-----
//';
//        $params->period = 'Annually';
//        $params->unique_id = 'un32ms0902';
//        $params->dcv_method = 'https';
//        $params->contact_email = '1342479179@qq.com';
//        $params->callback = 'https://xays.top';
//        $params->domains = 'www.xays.top';
//        $params->renew = 'no';

        $data = $this->client->addSSLOrder($params);

        $this->assertStringContainsString('success', $data->status);
    }
}
